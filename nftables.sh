...
configure_compatibility() {
    echo -e "${WHITE}配置与 iptables 的兼容性...${NC}"
...
ExecStop=/usr/sbin/iptables-save > /etc/iptables.rules
...
disable_nftables() {
    ...
    echo -e "${GREEN}nftables 已停用！${NC}"
}

# 添加卸载 nftables 函数
uninstall_nftables() {
    echo -e "${WHITE}正在卸载 nftables...${NC}"
...
main_menu() {
    while true; do
        ...
        case $choice in
            ...
            6)
                clear_screen
                echo -e "${CYAN}=== 停用 nftables ===${NC}\n"
                echo -n "确定要停用 nftables 吗？(y/n): "
                read -r confirm
                if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
                    disable_nftables
                else
            ...
            7)
                clear_screen
                echo -e "${CYAN}=== 卸载 nftables ===${NC}\n"
                echo -n "确定要卸载 nftables 吗？此操作将删除所有配置！(y/n): "
                read -r confirm
                if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
                    uninstall_nftables
                else
